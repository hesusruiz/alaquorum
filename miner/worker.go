...
func (w *worker) resultLoop() {
	for {
		...
		case block := <-w.resultCh:
			...
			fmt.Printf("JRM-Miner.resultLoop Writeblock number %v gasLimit %v\n", block.Number(), block.GasLimit())
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	// JRM-Miner.commitTransactions-Replaced the lines below
	...
	if w.isRunning() {
		...
		case w.taskCh <- &task{receipts: receipts, privateReceipts: privateReceipts, state: s, privateStateRepo: psrCopy, block: block, createdAt: time.Now()}:
			...
			log.Info("Commit new mining work", "number", block.Number(), "sealhash", w.engine.SealHash(block.Header()),
				...
				"gas", block.GasUsed(), "gasLimit", block.GasLimit(), "fees", totalFees(block, receipts),
