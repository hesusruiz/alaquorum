...
func (rt *RedTNode) displaySignersForBlockNumber(number uint64, latestTimestamp uint64) uint64 {
	...
	fmt.Printf("Block %v (%v sec %v), Author: %v (%v) (%v)\n", number, elapsed, currentTimestamp, oper.Operator, asProposer[author], author)
	...
	for _, item := range enodes {
		...
		if item.Address.Hex() == author.Hex() {
			fmt.Printf("X%4v %4v %15v (%v)\n", asProposer[item.Address], asSigner[item.Address], item.Operator, item.Address)
		} else {
			fmt.Printf(" %4v %4v %15v (%v)\n", asProposer[item.Address], asSigner[item.Address], item.Operator, item.Address)
		}
