...
func (c *core) catchUpRound(view *istanbul.View) {
	...
	// JRM-trace catchUpRound
	//	logger.Trace("Catch up round", "new_round", view.Round, "new_seq", view.Sequence, "new_proposer", c.valSet)
	currentValAddress := c.valSet.GetProposer().Address()
	currentValIndex, _ := c.valSet.GetByAddress(currentValAddress)
	nextValIndex := currentValIndex + 1
	if nextValIndex == c.valSet.Size() {
		nextValIndex = 0
	}
	nextValidator := c.valSet.GetByIndex(uint64(nextValIndex))
	logger.Trace("Catch up round", "new_round", view.Round, "new_seq", view.Sequence, "new_proposer", nextValidator)
	// JRM-trace catchUpRound END
